    ...
    class  << self
      ...
      def masters
        ...
        @zones.each do |zone|
          ...
          if node[:dns].has_key? :rndc_key and
            node[:dns][:rndc_key].blank? != true
            masters[name]["key"] = node[:dns][:rndc_key]
          end

          # use zone bag override if it exists
          if zone.has_key? "master_address" and
            zone["master_address"].blank? != true
          ...
          # validate
          unless masters[name].has_key? "key" and
            masters[name].has_key? "master"
            masters.delete(name)
          end
        end
      ...
      def keys
        ...
        if node.has_key? :dns and node[:dns].has_key? :rndc_key
          rkey = node[:dns][:rndc_key]
          k[rkey] = data_bag_item("rndc_keys", rkey)
        end
        ...
        @zones.each do |zone|
          ...
          if zone.has_key? "rndc_key"
            k[zone['rndc_key']] =
              data_bag_item("rndc_keys", zone["rndc_key"]).to_hash
          end
        end
        k
      end
      ...
      #
      def load_zones
        unless node.has_key? :dns and
          node[:dns].has_key? :zones and
          node[:dns][:zones].blank? != true
          return nil
        end
        ...
        node[:dns][:zones].each do |zone|
          ...
          b_name = Helpers::DataBags.escape_bagname(zone)
          zones << data_bag_item(bag_name, b_name).to_hash
        end
        @zones
      end
